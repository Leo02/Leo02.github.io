...
getElement('toggleMenu', 'id').addEventListener("click", function(e) {
    document.body.classList.toggle('showMenu');
});

if(getElement('post-content', 'class').length){
    getElement('.post-content img').forEach(function(item, i){
...
function webPReplace(){
	var postContent = getElement('post-content', 'class')[0],
		noscript = getElement('.post-content noscript')
	if(noscript.length){
		var content = getElement('.post-content noscript')[0].textContent;
		...
		if(images.length){
			Array.prototype.forEach.call(images,function(item, i){
				...
				var newSrc = src.replace('/upload/images', '/upload/w_' + postContent.offsetWidth + ',c_thumb,f_auto/images');

				item.width = postContent.offsetWidth;
				item.height = postContent.offsetWidth * 0.5;
				item.classList.add('lazy');
				item.setAttribute('data-normal', newSrc);
			});
		}

		postContent.innerHTML = postContent.innerHTML + el.getElementsByTagName('body')[0].innerHTML;
